...
Normfinder=function(filename,Groups=TRUE,ctVal=TRUE,pStabLim=0.25){
    ...
    if (ngr>1){   # More than one group.
        ...
        #
        ord=order(res[,3])
        FinalRes=list(Ordered=data.frame("GroupDif"=round(res[ord,1],2),
                                        "GroupSD"=round(res[ord,2],2),
                                        "Stability"=round(res[ord,3],2),
        ...
        if ((ncand>=2)&(ngenes>3)){
            ...
            ord=order(sigma)
            FinalRes=list(Ordered=data.frame("GroupSD"=round(sigma[ord],2),
                                            row.names=genenames[ord]),
                        PairOfGenes=data.frame("Gene1"=genenames[vv2[,1]],
                                            "Gene2"=genenames[vv2[,2]],
